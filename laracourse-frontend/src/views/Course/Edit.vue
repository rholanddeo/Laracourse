    ...
    <div class="border rounded-lg bg-white mb-5">
      ...
      <div class="relative overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-700">
          ...
          <tbody>
            <tr class="bg-white border-t" v-for="(material, index) in state.material" :key="material.id">
              <td class="px-6 py-4">
                {{ index + 1 }}.
              </td>
          ...
          <div class="p-4">
            <form @submit.prevent="createMateri">
              ...
              <div class="flex justify-end">
                <button type="submit"
                  class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2">Input</button>
              </div>
            </form>
          </div>
        </div>
  ...
  setup() {
    ...
    const updateCourse = async () => {
      try {
        ...
        toast.error("Course Failed to Update");
      }
    }
    ...
    const createMateri = async () => {
      try {
        ...
        state.value.errors = error.response.data.errors;
        toast.success("Course Added Failed");
      }
    }
